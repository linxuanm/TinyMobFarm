...
public class ItemLasso extends Item {
	...
	public boolean itemInteractionForEntity(ItemStack stack, PlayerEntity player, LivingEntity target, Hand hand) {
		...
		if (!player.world.isRemote()) {
			...
			mobData.put("Rotation", NBTHelper.createNBTList(
					DoubleNBT.func_229684_a_(0), DoubleNBT.func_229684_a_(0)));
	...
	public ActionResultType onItemUse(ItemUseContext context) {
		...
		if (!context.getWorld().isRemote()) {
			...
			DoubleNBT x = DoubleNBT.func_229684_a_(pos.getX() + 0.5);
			DoubleNBT y = DoubleNBT.func_229684_a_(pos.getY());
			DoubleNBT z = DoubleNBT.func_229684_a_(pos.getZ() + 0.5);
